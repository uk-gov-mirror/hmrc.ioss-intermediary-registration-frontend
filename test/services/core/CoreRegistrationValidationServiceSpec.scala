...
class CoreRegistrationValidationServiceSpec extends SpecBase with MockitoSugar with ScalaFutures with Matchers with BeforeAndAfterEach {
  ...
  private val request = AuthenticatedDataRequest(FakeRequest("GET", "/"), testCredentials, vrn, Enrolments(Set.empty),emptyUserAnswers, None, 1, None, None)

  implicit val dataRequest: AuthenticatedDataRequest[AnyContent] = AuthenticatedDataRequest(request, testCredentials, vrn, Enrolments(Set.empty), emptyUserAnswers, None, 1, None, None)
  ...
  "coreRegistrationValidationService.searchScheme" - {
    ...
    "call searchScheme with correct ioss number and must return match data" in {
      ...
      val value = coreRegistrationValidationService.searchScheme(iossNumber, countryCode).futureValue.get
    ...
    "must return None when no match found" in {
      ...
      val value = coreRegistrationValidationService.searchScheme(iossNumber, countryCode).futureValue
    ...
    "must return exception when server responds with an error" in {
      ...
      val response = intercept[Exception](coreRegistrationValidationService.searchScheme(iossNumber, countryCode).futureValue)
